...
fn main() {
    ...
    let mut numbers_next_to_gears: Vec<Vec<Number>> = vec![];
    ...
    for symbol in symbols {
        ...
        if gear_numbers.len() == 2 {
            numbers_next_to_gears.push(gear_numbers.clone());
        }
    ...
    println!(
        ...
        numbers_next_to_gears
            .iter()
            .map(|gear| {
                let mut result = gear[0].value;

                for number in gear.iter().skip(1) {
                    result *= number.value;
                }
